...
$sql = "SELECT * FROM classes WHERE school = ? ORDER BY created_at DESC LIMIT 4";
$stmt = $con->prepare($sql);
$stmt->bindParam(1, $_SESSION['id']);
$stmt->execute();
                ...
                <div style=" height:1200px;" class="flex flex-col w-3/4 bg-slate-50 overflow-scroll overflow-x-hidden">
                    ...
                    <div class="w-fill h-60 mx-10 mt-10 rounded-md bg-white shadow-md p-7 flex flex-row">
                        <div class="flex flex-col w-3/6 h-full justify-between">
                            <div>
                                <p class="text-xl font-medium">"<?= $data['school_name'] ?>" - Overview</p>
                    ...
                    <!--Show teachers list-->
                    <div class="w-fill h-40 mx-10 my-10 rounded-md bg-white shadow-md flex flex-col">
                        <div class="border-b-2 w-full h-14 flex pl-5 items-center justify-between flex flex-row">
                            ...
                            <p onclick="window.location.href='schoolTeachers.php'" class="font-medium text-sm mr-5 cursor-pointer">View all Teachers</p>
                        </div>
                    ...
                    <div class="w-fill h-40 mx-10 my-10 rounded-md bg-white shadow-md flex flex-col">
                        <div class="border-b-2 w-full h-14 flex pl-5 items-center justify-between flex flex-row">
                            ...
                            <p onclick="window.location.href='schoolClasses.php'" class="font-medium text-sm mr-5 cursor-pointer">View all Classes</p>
                        </div>
                        <!--The foreach loop to display the teachers-->
                        <div class="h-80 overflow-y-auto text-black h-fill">
                            <?php if ($dataClasses) : ?>
